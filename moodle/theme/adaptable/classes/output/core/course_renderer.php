...
class course_renderer extends \core_course_renderer {
    ...
    //ThuyetLV: Custom trang course
    public function course_category($category) {
        global $CFG, $OUTPUT, $PAGE;
        $tab = "courses";
//        var_dump($tab);die; //courses
        $renderable = new \block_category\output\main($tab, "7");
        $renderer = $this->page->get_renderer('block_category');
...
//        
////        $rtn =  parent::course_category($category);
////        return $rtn;
//        
...
//        return parent::render_from_template('core_course/courses-view',$rtn);
//        //return $rtn;
//    }
    ...
    protected function coursecat_coursebox_content(coursecat_helper $chelper, $course, $type = 3) {
        ...
        $content .= html_writer::link(new moodle_url('/course/view.php', array('id' => $course->id)), $contentimages . $contentfiles);
        ...
        if ($type == 2) {
            ...
            $coursename = $chelper->get_course_formatted_name($course);
            $content .= html_writer::tag('h3', html_writer::link(new moodle_url('/course/view.php', array('id' => $course->id)), $coursename, array('class' => $course->visible ? '' : 'dimmed')));
        }
    ...
    public function course_section_cm($course, &$completioninfo, cm_info $mod, $sectionreturn, $displayoptions = array()) {
        ...
        if ((method_exists($mod, 'is_visible_on_course_page')) && (!$mod->is_visible_on_course_page()) || (!$mod->uservisible && empty($mod->availableinfo))) {
            return $output;
        }
    ...
    protected function course_section_cm_get_meta(cm_info $mod) {
        ...
        if ($meta->isteacher) {
            ...
            if (!$meta->submissionnotrequired && $meta->numsubmissions !== false) {
                $engagementmeta[] = get_string('xofy' . $meta->submitstrkey, 'theme_adaptable', (object) array(
